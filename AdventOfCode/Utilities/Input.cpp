...
namespace Utilities
{
	...
	std::vector<uint8_t> SplitCommaSeparatedUInt8s(std::string_view input)
	{
		return Utilities::SplitStringAndTransform<uint8_t>(input, ",", [](std::string_view token) { return static_cast<uint8_t>(std::stoul(std::string{ token })); });
	}
